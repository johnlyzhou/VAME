...
def background(path_to_file, filename, file_format='.mp4', num_frames=1000):
    ...
    if not capture.isOpened():
        raise Exception(
            "Unable to open video file: {0}".format(os.path.join(path_to_file, "videos", filename + file_format)))
    ...
    np.save(os.path.join(path_to_file, "videos", filename + '-mean_background.npy'), bg)
...
def get_animal_frames(cfg, filename, pose_ref_index, start, length, subtract_background, file_format='.mp4',
                      crop_size=(300, 300)):
    ...
    if subtract_background:
        try:
            ...
            bg = np.load(os.path.join(path_to_file, "videos", filename + '-mean_background.npy'))
        except:
    ...
    for idx in tqdm.tqdm(range(length), disable=not True, desc='Align frames'):
        ...
        for i in pose_list:
            pose_list_bordered.append(
                (int(i[idx + start + lag][0] + crop_size[0]), int(i[idx + start + lag][1] + crop_size[1])))
