...
def background(path_to_file, filename, video_format='.mp4', num_frames=1000):
    ...
    if not capture.isOpened():
        raise Exception("Unable to open video file: {0}".format(
...
def alignment(path_to_file, filename, pose_ref_index, video_format, crop_size, confidence, frame_rate, 
              use_video=False, check_video=False, save_video=False):

    # read out data
    data = pd.read_csv(os.path.join(path_to_file, 'videos',
                       'pose_estimation', filename+'.csv'), skiprows=2)
    ...
    if save_video:
        save_aligned_video(frames, n, frame_count, frame_rate, crop_size)
